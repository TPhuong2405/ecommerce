...
import { WrapperLeft, WrapperRight, WrapperStyleHeader, WrapperListOrder, WrapperItemOrder, WrapperCountOrder, WrapperInfo, WrapperTotal, WrapperStyleHeaderDelivery } from './style';
import { DeleteOutlined, MinusOutlined, PlusOutlined } from '@ant-design/icons';
import img from '../../assets/images/anh1.webp';
          ...
          <WrapperLeft>
            ...
            <WrapperStyleHeader>
              <span style={{ display: 'inline-block', width: '390px' }}>
                <Checkbox onChange={handleOnchangeCheckAll} value={order?.product} style={{ marginRight: '10px' }} checked={listChecked?.length === order?.orderItems?.length} ></Checkbox>
                ...
                return (
                  <WrapperItemOrder key={item?.product}>
                    <div style={{ width: '390px', display: 'flex', alignItems: 'center', gap: 4 }}>
                      ...
                      <div style={{ width: '260px', overflow: 'hidden', textOverflow: 'ellipsis', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>{item?.name}</div>
                    </div>
                    <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <span>
                        <span style={{ fontSize: '13px', color: '#242424' }}>{convertPridce(item?.price)}</span>
                  ...
                  <WrapperCountOrder>
                      <button
                        ...
                        onClick={() => handleChangeCount('decrease', item?.product)}
                      >
                      ...
                      <button
                        style={{ border: 'none', background: 'transparent', cursor: 'pointer' }}
                        onClick={() => handleChangeCount('increase', item?.product)}
                      >
                        <PlusOutlined style={{ color: '#000', fontSize: '10px' }} />
                      </button>
                    </WrapperCountOrder>
                    <span style={{ color: 'rgb(255, 66, 78)', fontSize: '13px', fontWeight: 500 }}>
                      {convertPridce(item?.price * item?.amount)}
                    </span>
            ...
            <div style={{ width: '100%' }}>
              <WrapperInfo>
                <div>
                  ...
                  <span style={{ fontWeight: 'bold' }}>{`${user?.address} ${user?.city}`}</span><br />
              ...
              <WrapperInfo>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span>Tạm tính</span>
                  <span style={{ color: '#000', fontSize: '14px', fontWeight: 'bold' }}>{convertPridce(priceMemo)}</span>
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <span>Giảm giá</span>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <span style={{ color: '#000', fontSize: '14px', fontWeight: 'bold' }}>{convertPridce(diliveryPriceMemo)}</span>
                </div>
              </WrapperInfo>
              <WrapperTotal>
                <span>Tổng tiền</span>
                <span style={{ display: 'flex', flexDirection: 'column' }}>
                  <span style={{ color: 'rgb(254, 56, 52)', fontSize: '24px', fontWeight: 'bold' }}>{convertPridce(totalPriceMemo)}</span>
                  <span style={{ color: '#000', fontSize: '11px', fontWeight: 'bold' }}>(Đã bao gồm thuế VAT nếu có)</span>
                </span>
              </WrapperTotal>
            </div>
            <ButtonComponent
